...
//! says no); `PUT /api/upload/{id}/{index}` takes one raw chunk (16 MiB
//! cap, layered in `main.rs`); `POST /api/upload/{id}/finish` assembles the
...
async fn start(cx: &Cx, Json(input): Json<StartIn>) -> Result<Json<Answer<StartOut>>> {
    ...
    if let Some(id) = folder {
        match store.folder(id).await {
            ...
            Ok(_) => return Ok(Json(Err(Refusal::NotFound))),
            Err(_) => return Ok(Json(Err(Refusal::Unavailable))),
        }
    }
    match store
        ...
        .await
    {
        Ok(session) => Ok(Json(Ok(StartOut {
            id: session.id,
            chunk_size: session.chunk_size,
        }))),
        Err(error) => Ok(Json(Err(store_refusal(error)))),
    }
}
...
async fn chunk(cx: &Cx, body: Bytes) -> Result<Json<Answer<ChunkOut>>> {
    ...
    let index: u64 = match uri(cx).path().rsplit('/').next().and_then(|raw| raw.parse().ok()) {
        Some(index) => index,
        None => return Ok(Json(Err(Refusal::BadChunk))),
    };
    ...
    match store.record_chunk(&session.id, index, &body).await {
        Ok(session) => Ok(Json(Ok(ChunkOut {
